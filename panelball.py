...
class Game():
    difficulty = "Normal"
    chances_player_left = 3
    chances_player_right = 3
    score_player_left = 0
...
class Ball(VectorSprite):
    ...
    def update(self,seconds):
        ...
        if self.pos.x <= 0:
            Game.chances_player_left -= 1
            Game.score_player_right += 1
        elif self.pos.x >= Viewer.width:
            Game.chances_player_right -= 1
            Game.score_player_left += 1
        if self.side == "left" and self.pos.x >= Viewer.width/2:
            Game.passed_middle += 1
            self.side = "right"
        elif self.side == "right" and self.pos.x <= Viewer.width/2:
            Game.passed_middle += 1
            self.side = "left"
        #print(Game.passed_middle,self.side)
        if Game.passed_middle == 3:
            self.move *= 1.1
            Game.passed_middle = 0
            Game.stufe += 1
...
class Panel(VectorSprite):
    ...
    def create_image(self):
        ...
        pygame.draw.rect(self.image, (1,1,1),(0,0,30,300))
        self.image.set_colorkey((0,0,0))
        self.image.convert_alpha()
        self.image0 = self.image.copy()
        self.rect = self.image.get_rect()
#        self.image.fill((0,0,0))
    ...
    def update(self, seconds):
        ...
        if self.pos.y >= -150:
            self.pos.y = -150
        elif  self.pos.y < -Viewer.height + 150:
            self.pos.y = -Viewer.height + 150
class Viewer(object):
    width = 0
    height = 0
    images={}
    ...
    def prepare_sprites(self):
        ...
        self.panelgroup = pygame.sprite.Group()
        self.ballgroup = pygame.sprite.Group()
        
        VectorSprite.groups = self.allgroup
        Flytext.groups = self.allgroup, self.flytextgroup
        Ball.groups = self.allgroup, self.ballgroup
        Panel.groups = self.allgroup, self.panelgroup
        #Flytext(Viewer.width/2,Viewer.height/4,"This is an example Flytext.", (0,0,255), duration=10, fontsize=100, dy=-40)
        m = pygame.math.Vector2(500,0)
        m.rotate_ip(random.randint(0,360))
        self.ball1 = Ball(pos=pygame.math.Vector2(1000,-500),move=m)
        self.panel1 = Panel(pos=pygame.math.Vector2(200,-500))
        self.panel2 = Panel(pos=pygame.math.Vector2(1200,-500))
    ...
    def run(self):
        ...
        while running:
            ...
            for event in pygame.event.get():
                ...
                # ------- pressed and released key ------
                    ...
                    if event.key == pygame.K_ESCAPE:
                        ...
                        self.ball1.pos = pygame.math.Vector2(1000,-500)
                        self.ball1.move = pygame.math.Vector2(500,0)
            ...
            if pressed_keys[pygame.K_UP]:
                self.panel2.pos += pygame.math.Vector2(0,5)
            if pressed_keys[pygame.K_DOWN]:
                self.panel2.pos += pygame.math.Vector2(0,-5)
            if pressed_keys[pygame.K_s]:
                self.panel1.pos += pygame.math.Vector2(0,5)
            ...
            if Game.stufe == 3:
                ...
                m = pygame.math.Vector2(500,0)
                m.rotate_ip(random.randint(0,360))
                Ball(pos=pygame.math.Vector2(1000,-500),move=m)
                for b in self.ballgroup:
                    b.move.scale_to_length(500)
            for b in self.ballgroup:
                print(b.move.length())
            if Game.chances_player_right == 0 and not self.endgame:
                self.endgame_time = self.playtime + 5
                self.endgame = True
                Flytext(x = Viewer.width//2, y = Viewer.height//3, text = "The left player wins!", fontsize = 100)
            elif Game.chances_player_left == 0 and not self.endgame:
                self.endgame_time = self.playtime + 5
                self.endgame = True
                Flytext(x = Viewer.width//2, y = Viewer.height//3, text = "The right player wins!", fontsize = 100)
            if self.playtime > self.endgame_time and self.endgame == True:
                break
            ...
            write(self.screen, "FPS: {}".format( #{:8.3}
                self.fps ), x=10, y=10)
            write(self.screen, "Score:  {} - {}".format(Game.score_player_left,Game.score_player_right), x = Viewer.width//2 - 50, y = Viewer.height - 100)
            write(self.screen, "Chances:{} - {}".format(Game.chances_player_left,Game.chances_player_right), x = Viewer.width//2 - 50, y = Viewer.height - 50)
            write(self.screen, "Level:  {}".format(Game.level), x = Viewer.width//2 - 50, y = Viewer.height - 50)
            ...
            for ball in self.ballgroup:
                crashgroup = pygame.sprite.spritecollide(ball, self.panelgroup, False, pygame.sprite.collide_mask)
                for panel in crashgroup:
                    #elastic_collision(ball,panel)
                    #ball.move *= -1
                        if ball.pos.y >= panel.pos.y - 150 and ball.pos.y <= panel.pos.y + 150:
                            print("linker Rand")
                            ball.move.x *= -1
                        else:
                            ball.pos.y *= -1
